...
def randomizedMotifSearch(dnaList, k, t):
    ...
    while(True):
        ...
        print("motifs: ", motifs, " score: ", score) 
        if score < bestScore:
            bestMotifs = motifs
            bestScore = score
        else:
...
def formProfile(motifList):
    ...
    for i in matrix:
        for j in range(k):
            matrix[i].append(0) # the 1 is a 0 in problem 24

    for kmer in motifList:
        for i in range(len(kmer)):
            for key in matrix:
                if kmer[i] == key:
                    matrix[key][i] += (1/((len(motifList)))) # the +4 does not exist in problem 24
                else:
                    matrix[key][i] += (0.0)
    return(matrix)
...
print(randomizedMotifSearch(dnaList, k, t))
